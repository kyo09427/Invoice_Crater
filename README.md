...
- **PDF生成** 🆕 v1.2.0
  ...
  - **明細の並び替え機能**（8種類のソートオプション）
    - 支払日順（古い順・新しい順）
    - 支払先順（A→Z・Z→A）
    - 金額順（安い順・高い順）
    - 用途順（A→Z・Z→A）
  - プレビュー機能付き
  - 明細テーブルはWeb（Tailwind）風のデザイン
  - 決済手段列は支払方法ごとにセル全体を自動で色分け
  - 共有・印刷機能
  - エラーハンドリング強化
...
│                 │  │                 │  │  🆕 並び替え機能 │
│  📄 11月度精算書 │  │  タイトル: ____  │  │  [並び順: 支払日↑]│
│     2024/11/15  │  │  申請者: ______  │  │                 │
│     ¥12,500     │  │  作成日: ______  │  │   精 算 書      │
│                 │  │                 │  │                 │
│  📄 10月度精算書 │  │  ───────────────  │  │  申請者: 山田太郎 │
│     2024/10/20  │  │  11/01 〇〇商店  │  │  作成日:2024/11 │
│     ¥8,300      │  │        文房具     │  │                 │
│                 │  │        ¥1,200    │  │  [明細テーブル]  │
│  [+] 新規作成    │  │                 │  │                 │
└─────────────────┘  └─────────────────┘  │  合計: ¥12,500  │
...
│   │   ├── expense_pdf_preview_screen.dart     # PDFプレビュー 🆕 並び替え機能
...
2. PDFプレビュー画面が表示されます
3. **並び替え機能** 🆕 v1.2.0
   - 右上の「並び替え」アイコン（ソートアイコン）をタップ
   - 8種類のソートオプションから選択：
     - **支払日順**: 古い順 / 新しい順
     - **支払先順**: A→Z / Z→A
     - **金額順**: 安い順 / 高い順
     - **用途順**: A→Z / Z→A
   - 選択した順序でPDFがリアルタイムに再生成されます
   - 現在の並び順は画面上部に表示されます
4. 共有ボタンで他のアプリに送信、または印刷
...
A: v1.0では未実装です。v1.2で追加予定です（ロードマップ参照）。
...
**Q: PDF内の明細の順序を変更できますか？** 🆕

A: はい、v1.2.0から並び替え機能が追加されました。
- PDFプレビュー画面の右上にある「並び替え」ボタンをタップ
- 8種類のソートオプション（支払日、支払先、金額、用途の昇順・降順）から選択
- 選択した順序でPDFがリアルタイムに再生成されます
- ファイル名にもソート順が反映されます（例：`11月度精算書_支払日↑.pdf`）

**Q: 並び替えた順序は保存されますか？** 🆕

A: 現在のバージョン（v1.2.0）では、並び替え順序は画面を閉じるとリセットされます。v1.3で並び順の保存機能を追加予定です。

## 🆕 最新の変更点 (v1.2.0)

### 2024年12月 - PDF並び替え機能追加アップデート

#### ✅ PDF並び替え機能の実装 🎉

**新機能**: PDFプレビュー画面で明細の表示順序を変更できるようになりました
...
- 並び順の保存機能なし（v1.3で実装予定）
- データのバックアップ・復元機能なし（v1.3で実装予定）
- 削除の取り消し機能なし（v1.3で実装予定）
...
1. **Enumに追加**
   ...
   `lib/presentation/screens/expense_pdf_preview_screen.dart`：

   ```dart
   enum PdfSortType {
     // ... 既存のオプション
     categoryAsc('カテゴリ (A→Z)', Icons.category),
     categoryDesc('カテゴリ (Z→A)', Icons.category);
...
2. **ソート処理を実装**
   ...
   List<ExpenseItem> _sortItems(List<ExpenseItem> items) {
     ...
     switch (_currentSortType) {
       // ... 既存のケース
       case PdfSortType.categoryAsc:
         sortedItems.sort((a, b) => a.category.compareTo(b.category));
         break;
       case PdfSortType.categoryDesc:
